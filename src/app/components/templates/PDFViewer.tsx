// src/app/components/templates/PDFViewer.tsx
"use client";
...
export const PDFViewer: React.FC<PDFViewerProps> = ({ file, regions }) => {
  ...
  useEffect(() => {
    if (regions && regions.length > 0 && overlayCanvasRef.current && imageData) {
      drawBoundingBoxes();
    }
  }, [regions, scale, imageData]);
  ...
  const loadPDFAsImage = async () => {
    try {
      // Load PDF.js from CDN using script tag (more reliable)
