...
export const ApiConfigProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    async function fetchApiConfig() {
      try {
        ...
        if (data.ip) {
          ...
          // setAIBaseUrl(`http://${data.ip}:8080`);
          
          // For remote use below lines
          setOcrApiUrl(`https://kkti3idqzhgqny-8080.proxy.runpod.net/run-ocr`);
          setTestTemplateApiUrl(`https://h86md9lnjv592j-19123-8080.proxy.runpod.net/api/templates/test`);
          setAIBaseUrl(`https://h86md9lnjv592j-19123-8080.proxy.runpod.net`)
  ...
  return (
    <ApiConfigContext.Provider value={{ ocrApiUrl, baseUrl, isLoading, error, testTemplateApiUrl, aiBaseUrl }}>
      {children}
