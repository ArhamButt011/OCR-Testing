...
async function getHandler(req: Request | any, context?: any) {
    try {
        ...
        const primaryIpData = await collection.findOne(
            { type: "primary" },
            { projection: { ip: 1, remember: 1, _id: 0 } }
        );
        const secondaryIpData = await collection.findOne(
            { type: "secondary" },
...
async function postHandler(req: Request | any, context?: any) {
    try {
        ...
        if (!ip) {
            return NextResponse.json(
                { error: "Primary IP address is required" },
                { status: 400 }
            );
        }
