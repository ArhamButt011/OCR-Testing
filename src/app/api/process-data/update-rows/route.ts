...
// Helper function to convert empty strings to null for Oracle
function sanitizeValue(value: string | number | null | undefined): string | number | null {
...
function sanitizeNumeric(value: string | number | null | undefined): number | null {
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    for (const job of jobsToUpdate) {
      ...
      if (!fileId) {
        ...
        const fileTableCurrent = `XTI_${currentYear}_T`;
        const fileTablePrevious = `XTI_${previousYear}_T`;
  ...
  `;
      ...
      // Sanitize all values before sending to Oracle
      const sanitizedData = {
        bolNo: sanitizeValue(job.blNumber?.toString()),
        issQty: sanitizeNumeric(job.totalQty),
        rcvQty: sanitizeNumeric(job.received),
        podDate: sanitizeValue(job.podDate),
        sign: job.podSignature === "yes" ? "Y" : "N",
        symtNone: 0,
        symtDamg: sanitizeNumeric(job.damaged),
        symtShrt: sanitizeNumeric(job.short),
        symtOrvg: sanitizeNumeric(job.over),
        symtRefs: sanitizeNumeric(job.refused),
        symtSeal: sanitizeValue(job.sealIntact) || "N",
        fileId,
        crtdDtt,
      };
